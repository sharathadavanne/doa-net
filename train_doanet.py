...
def run_epoch(data_generator, optimizer, model, hnet_model, activity_loss, criterion, metric_cls, params, device, is_train=True):
    ...
    return optimizer, model, metric_cls, train_loss, train_dMOTP_loss, train_dMOTA_loss, train_act_loss
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            # ---------------------------------------------------------------------
            model.train()
            train_metric = doa_metric()
            optimizer, model, train_metric, train_loss, train_dMOTP_loss, train_dMOTA_loss, train_act_loss = run_epoch(data_gen_train, optimizer, model, hnet_model, activity_loss, criterion, train_metric, params, device)

            if params['use_hnet'] and not params['use_dmot_only']:
                train_hung_loss, train_recall_doa, train_precision_doa = train_metric.get_results()
            else:
                train_hung_loss = train_metric.get_results()
            ...
            with torch.no_grad():
                optimizer, model, val_metric, val_loss, val_dMOTP_loss, val_dMOTA_loss, val_act_loss = run_epoch(data_gen_val, optimizer, model, hnet_model, activity_loss, criterion, val_metric, params, device, is_train=False)
            ...
            print(
                ...
                'train_hung_loss: {:0.3f}{}, val_hung_loss_deg: {:0.3f}{}, '
                'best_val_epoch: {}'.format(
                    ...
                    180*train_hung_loss/np.pi, '/{:0.2f}/{:0.2f}'.format(train_precision_doa*100.0, train_recall_doa*100.0) if params['use_hnet'] and not params['use_dmot_only'] else '',
        ...
        with torch.no_grad():
            optimizer, model, test_metric, test_loss, test_dMOTP_loss, test_dMOTA_loss, test_act_loss = run_epoch(data_gen_test, optimizer, model, hnet_model, activity_loss, criterion, test_metric, params, device, is_train=False)
